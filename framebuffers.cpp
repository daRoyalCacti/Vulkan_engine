...
void Framebuffers::setup() {
    ...
    for (size_t i = 0; i < no_framebuffers; i++) {
        ...
        framebufferInfo.attachmentCount = 1;                                    //the number of elements in the pAttachments array
        framebufferInfo.pAttachments = &image_views.swapChainImageViews[i];     //the image views that should be used -- want this to be the same as the swap chain
